...
| 6 | Data | **DB rows completed 2026-07-14; upload namespaces completed 2026-07-15** | Cascades remove task-owned rows and response values; new task objects are isolated from general uploads; reference-aware S3 object reclamation remains open |
...
**S3 cleanup still open:** attachment duplication creates distinct metadata rows that intentionally share an immutable object URL. Calling the existing object-delete action when either task is removed could therefore break the remaining copy. The remaining safe step is a reference-aware sweep: enumerate live task/update/response attachment URLs, list only the three task-owned prefixes, and delete only objects absent from every live metadata row after a grace period. Until that exists, row integrity and future object ownership are fixed but unreferenced object bytes may remain.
...
8. **Partially completed 2026-07-15:** ~~schema cascades for task-owned children + hosted push (2.3); isolate all new task uploads under task-owned prefixes.~~ Implementing the reference-aware orphan sweep across those prefixes remains open; legacy unscoped objects are intentionally excluded.
