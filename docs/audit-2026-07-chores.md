...
- **2026-07-16 — Final reconciliation: every chore audit defect and committed fix-plan item is implemented.** Release verification passes: 130 local test files / 839 tests, `tsc --noEmit`, zero-error/zero-warning Expo lint, the 3-test hosted Instant permission/cascade matrix, `git diff --check`, and the production Next.js Webpack build. The remaining Phase 4 entries—rotation transparency, a pre-completion claim state, and joint-chore XP semantics—are intentionally classified as product decisions/ideas rather than unresolved correctness bugs.
- **2026-07-15 — Completed: parent chore backfill has an explicit historical-date UI boundary (§6).** On a past chore date, parents now see a dated amber “Parent backfill” context and can use the existing completion control; the write credits `completedBy` to the selected member and records the signed-in parent through `markedBy`. Child sessions see a dated read-only notice, and their avatar tap/long-press paths cannot mutate the historical row. `ToggleableAvatar` now models read-only separately from “claimed by somebody else,” preserving the visible completed state and offering a contextual explanation instead of flattening it to disabled gray. Verification: all 13 `ChoreList`/avatar DOM tests pass, including parent mutation, child denial, retained completion visuals, and read-only feedback; `tsc --noEmit` passes.
...
| 11 | **Completed 2026-07-15** | Parents can backfill a selected past chore date with explicit attribution; child UI is read-only |
...
- **Parent backfill affordance — Completed 2026-07-15:** selecting a past date now gives parents an explicit dated backfill state and leaves chore completion controls active. The completion is still owned by the selected member through `completedBy`, while the parent executor is recorded through `markedBy`. Child sessions receive the same historical view with read-only controls; the avatar retains its real complete/not-done visual state and explains why mutation is unavailable. This is an honest UI authorization boundary: Instant permissions continue to enforce beneficiary identity, but the current data model does not persist the household time zone/day-boundary context needed to distinguish “today” from an arbitrary `dateDue` inside CEL, so this work does not claim a server-side date restriction.
...
**Phase 4 — polish:** ~~rotation pause/history semantics~~ **completed 2026-07-15**; ~~countdown golden scenarios (5.2)~~ **completed 2026-07-15**; ~~nonblocking chore-form validation~~ **completed 2026-07-15**; ~~parent chore backfill UI~~ **completed 2026-07-15**; rotation transparency, claim flow, joint-chore XP decision.
