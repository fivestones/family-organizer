...
const basePollMs = parsePositiveMs(process.env.APPLE_CALDAV_POLL_BASE_SECONDS, DEFAULT_BASE_POLL_MS / 1000);
const errorPollMs = parsePositiveMs(process.env.APPLE_CALDAV_POLL_ERROR_SECONDS, DEFAULT_ERROR_POLL_MS / 1000);
const maxPollMs = parsePositiveMs(process.env.APPLE_CALDAV_POLL_MAX_IDLE_SECONDS, DEFAULT_MAX_POLL_MS / 1000);
...
async function main() {
    ...
    while (keepRunning) {
        try {
            ...
            const suggestedMs = Number(result.data?.nextPollInMs);
            const sleepMs = clamp(
                Number.isFinite(suggestedMs) && suggestedMs > 0 ? suggestedMs : basePollMs,
                basePollMs,
                maxPollMs
            );
            const label = result.data?.reason || (result.data?.skipped ? 'skipped' : 'synced');
            console.log(
                `[${timestamp()}] Apple Calendar poll ${result.status} ${label} next=${sleepMs}ms`
            );
            await sleep(sleepMs);
        } catch (error) {
